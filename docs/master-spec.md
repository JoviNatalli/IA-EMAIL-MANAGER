...
## Decisões de execução (Fase 3, 2026-09-05)

- **Rebrand**: o produto passou a chamar-se **Nuvoly** (antes "MailMind", conforme o brand original em §6) — já existia outro SaaS a usar o nome "MailMind" no mercado. Todas as strings de marca no código, README, dados de seed e metadata foram atualizadas (commit `969eddd`); as secções 1-64 acima ficam intocadas (registo histórico do brief original).
- **Outlook/Microsoft (§4, §44)**: decidido manter fora do roadmap por agora — só Gmail na Fase 3. Documentado como "Future Improvements" no README; a arquitetura de acesso a email deve ficar abstraída (interface tipo `EmailProvider`) para não exigir reescrita se Outlook for adicionado depois.
- **Gmail real implementado (commit `3ad2899`)**: OAuth Google (`access_type=offline`+`prompt=consent` para garantir refresh_token), sincronização inicial limitada a 30 threads + labels do utilizador (§28 SyncState → tabela `gmail_sync`), envio/rascunhos reais via Gmail API quando ligado (senão mantém-se o envio simulado da Fase 2/Demo Mode — §47), ações do dia a dia (estrela/lido/arquivar/labels importadas) propagadas de forma best-effort, mover-para-lixo/apagar/enviar/responder falham de forma visível em vez de fingir sucesso (§35). Scope explicitamente fora desta fase (documentado no README "Future Improvements", não escondido): sync incremental via `historyId` (já guardado, não usado), anexos reais, labels novas criadas na app → Gmail, renderização de HTML de emails (sempre convertido para texto simples), encriptação de tokens em repouso.
- **Bug de account-linking do Auth.js (encontrado e corrigido no teste manual, commit `60d8846`)**: o Auth.js liga a conta OAuth ao utilizador da SESSÃO ATIVA no momento do callback em vez de criar sempre um utilizador novo. Corrigido em `signInWithGoogle()` (`src/app/actions/auth.ts`): força `signOut` antes de iniciar o fluxo Google, sempre — nunca reintroduzir esse comportamento.
- Fase 3 confirmada completa e testada manualmente: login Google cria utilizador novo corretamente, inbox vazia até à primeira sincronização, sync/estrela/arquivar/responder confirmados a propagar de verdade para o Gmail real.
